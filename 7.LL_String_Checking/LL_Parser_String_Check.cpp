...
bool isStringAcceptableByLLParser(int indx, string &str, char startSymbol, map<char, map<char, set<int>>> &llparsingTable, vector<string> &productions)
{
    stack<char> st;
    st.push('$');
    st.push(startSymbol);
    while (!st.empty())
    {
        ...
        if (llparsingTable[curr][str[indx]].empty())
            return false;
        string prod = productions[*llparsingTable[curr][str[indx]].begin()];
        string rhs = prod.substr(3);
        for (int i = (int)rhs.size() - 1; i >= 0; i--)
        {
            st.push(rhs[i]);
        }
        while (!st.empty() && indx < str.size() && st.top() == str[indx])
        {
            indx++;
            st.pop();
        }
    }
    return indx == str.size();
}
...
int main()
{
    ...
    for (map<char, map<char, set<int>>>::iterator it = llparsingTable.begin(); it != llparsingTable.end(); it++)
    {
        for (map<char, set<int>>::iterator jt = it->second.begin(); jt != it->second.end(); jt++)
        {
            if (jt->second.size() > 1)
            {
                cout << "\n ----|Given CFG Is Not Suitable For LL(1) Parser!|-----\n";
                return 0;
            }
        }
    }
    ...
    while (1)
    {
        ...
        str += "$";
        cout << "Hello\n";
        if (isStringAcceptableByLLParser(0, str, startSymbol, llparsingTable, productions))
        {
            cout << "Given String Is Acceptable By LL Parser! \n";
        }
        else
        {
            cout << "Given String Is Not Acceptable By LL Parser! \n";
        }
    }
    return 0;
}
